# bot.py - النسخة المحدثة مع نظام الدفع المباشر
import telebot
import requests
import json
import os
...
# حساب استلام النجوم (المطور)
STAR_RECEIVER = "kayo_i"  # يوزر المطور الذي يستلم النجوم
...
def add_purchase_request(user_id: int, bot_id: str, plan_type: str, stars: int):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT INTO purchase_requests (user_id, bot_id, plan_type, stars, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                  (user_id, bot_id, plan_type, stars, "pending", datetime.now().isoformat()))
        conn.commit()
        conn.close()
        return True
    except:
...
def get_purchase_requests(user_id: int = None):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        if user_id:
            c.execute("SELECT * FROM purchase_requests WHERE user_id = ? ORDER BY created_at DESC", (user_id,))
        else:
            c.execute("SELECT * FROM purchase_requests WHERE status = 'pending' ORDER BY created_at DESC")
        results = c.fetchall()
        conn.close()
        return results
    except:
...
def update_purchase_status(request_id: int, status: str):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
...
💡 عند اختيار الباقة، ستظهر لك طريقة الدفع
"""
        try:
...
━━━━━━━━━━━━━━━━━━
📌 <b>طريقة الدفع:</b>
• أرسل النجوم إلى الحساب: @{STAR_RECEIVER}
• ثم اضغط على زر "تم الدفع"
...
def handle_duration(call):
    ...
    if duration_type == "unlimited" or is_admin(user_id):
        ...
        # طلب الدفع بالنجوم - طريقة جديدة مثل الصور
        add_pending_payment(user_id, bot_id, cost, duration_type)
        
        # إنشاء رسالة الدفع مثل الصور
        payment_text = f"""
...
━━━━━━━━━━━━━━━━━━
<b>طريقة الدفع:</b>
• أرسل {cost} نجوم إلى الحساب: @{STAR_RECEIVER}
• ثم اضغط على زر "تم الدفع" ✅

━━━━━━━━━━━━━━━━━━
⚠️ سيتم تفعيل بوتك تلقائياً بعد التأكيد
"""
        keyboard = telebot.types.InlineKeyboardMarkup(row_width=1)
        keyboard.add(
            create_colored_button("✅ تم الدفع", callback_data=f"confirm_payment:{bot_id}", style="success"),
...
def handle_confirm_payment(call):
    ...
    # التحقق من وجود طلب دفع معلق
    pending = get_pending_payment(user_id)
    if not pending:
        bot.answer_callback_query(call.id, "❌ لا يوجد طلب دفع معلق", show_alert=True)
        return
    ...
    if bot_data_db:
        ...
        if start_bot_process(bot_id, bot_file):
            ...
            # تسجيل المعاملة
            add_star_transaction(user_id, pending["stars"], "payment", f"دفع {pending['stars']} نجوم لتشغيل بوت {bot_id}")
            stats_data["stats"]["total_stars_earned"] = stats_data["stats"].get("total_stars_earned", 0) + pending["stars"]
            save_all()
...
"""
    ...
    # للمستخدمين العاديين - عرض رسالة الدفع مثل الصور
...
━━━━━━━━━━━━━━━━━━
<b>طريقة الدفع:</b>
• أرسل {duration['stars']} نجوم إلى الحساب: @{STAR_RECEIVER}
• ثم اضغط على زر "تم الدفع" ✅

━━━━━━━━━━━━━━━━━━
⚠️ سيتم تفعيل اشتراكك تلقائياً بعد التأكيد
"""
    keyboard = telebot.types.InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        create_colored_button("✅ تم الدفع", callback_data=f"confirm_subscription:{duration['plan']}", style="success"),
        create_colored_button("🔙 رجوع", callback_data="back", style="primary")
    )
...
def handle_confirm_subscription(call):
    user_id = call.from_user.id
    ...
    if pending["bot_id"] != "publish_subscription":
        bot.answer_callback_query(call.id, "❌ بيانات غير صحيحة", show_alert=True)
        return
    ...
    if add_user_subscription(user_id, days, sub_type):
        add_star_transaction(user_id, pending["stars"], "subscription", f"اشتراك {sub_type} - {pending['stars']} نجوم")
        stats_data["stats"]["total_stars_earned"] = stats_data["stats"].get("total_stars_earned", 0) + pending["stars"]
        save_all()
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("confirm_subscription:"))
def confirm_subscription_callback(call):
    handle_confirm_subscription(call)
...
if __name__ == "__main__":
    ...
    logger.info(f"⭐ حساب استلام النجوم: @{STAR_RECEIVER}")
    logger.info("=" * 60)
