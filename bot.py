# bot.py - النسخة المحدثة مع نظام تسعير الاشتراكات
import telebot
import requests
import json
import os
...
# حساب المطور لتحويل النجوم إليه
STAR_RECEIVER_ID = 7947679527  # ايدي المطور @ggzh9
...
def init_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS pending_payments (user_id INTEGER PRIMARY KEY, bot_id TEXT, stars INTEGER, plan_type TEXT, created_at TEXT)''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS subscription_prices (plan_type TEXT PRIMARY KEY, stars INTEGER)''')
    
    # إضافة الأسعار الافتراضية
    c.execute("INSERT OR IGNORE INTO subscription_prices (plan_type, stars) VALUES ('يوم', 10)")
    c.execute("INSERT OR IGNORE INTO subscription_prices (plan_type, stars) VALUES ('اسبوع', 50)")
    c.execute("INSERT OR IGNORE INTO subscription_prices (plan_type, stars) VALUES ('شهر', 600)")
...
# ==================== دوال الأسعار ====================
def get_price(plan_type: str) -> int:
    """الحصول على سعر خطة معينة"""
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT stars FROM subscription_prices WHERE plan_type = ?", (plan_type,))
        result = c.fetchone()
        conn.close()
        if result:
            return result[0]
        return DEFAULT_PRICES.get(plan_type, 10)
    except:
        return DEFAULT_PRICES.get(plan_type, 10)

def update_price(plan_type: str, stars: int) -> bool:
    """تحديث سعر خطة معينة"""
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("REPLACE INTO subscription_prices (plan_type, stars) VALUES (?, ?)", (plan_type, stars))
        conn.commit()
        conn.close()
        return True
    except:
...
def get_all_prices() -> dict:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT plan_type, stars FROM subscription_prices")
        results = c.fetchall()
        conn.close()
        prices = {}
        for plan_type, stars in results:
            prices[plan_type] = stars
        return prices
    except:
...
def add_pending_payment(user_id: int, bot_id: str, stars: int, plan_type: str):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("REPLACE INTO pending_payments (user_id, bot_id, stars, plan_type, created_at) VALUES (?, ?, ?, ?, ?)",
                  (user_id, bot_id, stars, plan_type, datetime.now().isoformat()))
        conn.commit()
        conn.close()
        return True
    except:
...
def get_pending_payment(user_id: int):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT bot_id, stars, plan_type FROM pending_payments WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        conn.close()
        if result:
            return {"bot_id": result[0], "stars": result[1], "plan_type": result[2]}
        return None
    except:
        return None
...
def admin_panel_keyboard():
    ...
    keyboard.add(
        ...
        create_colored_button("💰 تسعير الاشتراكات", callback_data="pricing_menu", style="primary")
    )
    keyboard.add(
...
def publish_menu_keyboard():
    ...
    day_price = get_price("يوم")
    week_price = get_price("اسبوع")
    month_price = get_price("شهر")
    
    keyboard.add(
        create_colored_button(f"📅 يوم ({day_price} ⭐)", callback_data="publish_day", style="primary"),
        create_colored_button(f"📅 أسبوع ({week_price} ⭐)", callback_data="publish_week", style="success"),
        create_colored_button(f"📅 شهر ({month_price} ⭐)", callback_data="publish_month", style="danger")
    )
...
def pricing_menu_keyboard():
    ...
    keyboard.add(
        create_colored_button(f"📅 يوم: {day_price} ⭐", callback_data="price_day", style="primary"),
        create_colored_button(f"📅 أسبوع: {week_price} ⭐", callback_data="price_week", style="success")
    )
    keyboard.add(
        create_colored_button(f"📅 شهر: {month_price} ⭐", callback_data="price_month", style="danger"),
        create_colored_button("🔄 إعادة تعيين الأسعار", callback_data="price_reset", style="primary")
    )
    keyboard.add(
...
<b>⭐ الأسعار الحالية:</b>
يوم: {day_price}⭐ | أسبوع: {week_price}⭐ | شهر: {month_price}⭐
...
def process_requirements_file(message, bot_id, bot_folder, bot_name):
    ...
    try:
        ...
        keyboard = telebot.types.InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            create_colored_button(f"📅 يوم ({day_price} ⭐)", callback_data=f"duration_day:{bot_id}", style="primary"),
            create_colored_button(f"📅 أسبوع ({week_price} ⭐)", callback_data=f"duration_week:{bot_id}", style="success")
        )
        keyboard.add(
            create_colored_button(f"📅 شهر ({month_price} ⭐)", callback_data=f"duration_month:{bot_id}", style="danger"),
        ...
        bot.reply_to(
            ...
            f"⭐ الأسعار بالنجوم:\n"
            f"• يوم: {day_price} نجوم\n"
            f"• أسبوع: {week_price} نجوم\n"
            f"• شهر: {month_price} نجوم\n"
...
⭐ <b>الباقات المتاحة:</b>
• يوم: {day_price} نجوم
• أسبوع: {week_price} نجوم  
• شهر: {month_price} نجوم

💡 سيتم خصم النجوم مباشرة من رصيدك
"""
        try:
...
• 🟢 <b>يوم</b> — {day_price} نجوم
• 🔵 <b>أسبوع</b> — {week_price} نجوم
• 🟣 <b>شهر</b> — {month_price} نجوم
• 💎 <b>غير محدود</b> — للمطورين فقط

━━━━━━━━━━━━━━━━━━
📌 <b>طريقة الدفع:</b>
• اختر الباقة المناسبة
• سيتم خصم النجوم تلقائياً من رصيدك
• سيتم تفعيل اشتراكك فوراً
...
"""
    ...
    if call.data == "statistics":
        ...
        msg = (
            ...
            f"<b>⭐ الأسعار الحالية:</b>\n"
            f"يوم: {day_price}⭐ | أسبوع: {week_price}⭐ | شهر: {month_price}⭐"
        )
        try:
...
━━━━━━━━━━━━━━━━━━
📌 لتغيير سعر خطة:
اضغط على الخطة التي تريد تغيير سعرها
"""
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=pricing_menu_keyboard())
        except:
            pass
        bot.answer_callback_query(call.id)
        return
    
    if call.data == "price_day":
        if not is_admin(user_id):
            bot.answer_callback_query(call.id, "❌ غير مصرح", show_alert=True)
            return
        msg = bot.edit_message_text("📝 أرسل السعر الجديد ليوم (عدد النجوم):", call.message.chat.id, call.message.message_id, reply_markup=back_to_admin())
        bot.register_next_step_handler(msg, process_update_price, "يوم")
        bot.answer_callback_query(call.id)
        return
    
    if call.data == "price_week":
        if not is_admin(user_id):
            bot.answer_callback_query(call.id, "❌ غير مصرح", show_alert=True)
            return
        msg = bot.edit_message_text("📝 أرسل السعر الجديد لأسبوع (عدد النجوم):", call.message.chat.id, call.message.message_id, reply_markup=back_to_admin())
        bot.register_next_step_handler(msg, process_update_price, "اسبوع")
        bot.answer_callback_query(call.id)
        return
    
    if call.data == "price_month":
        if not is_admin(user_id):
            bot.answer_callback_query(call.id, "❌ غير مصرح", show_alert=True)
            return
        msg = bot.edit_message_text("📝 أرسل السعر الجديد لشهر (عدد النجوم):", call.message.chat.id, call.message.message_id, reply_markup=back_to_admin())
        bot.register_next_step_handler(msg, process_update_price, "شهر")
        bot.answer_callback_query(call.id)
        return
    
    if call.data == "price_reset":
        if not is_admin(user_id):
            bot.answer_callback_query(call.id, "❌ غير مصرح", show_alert=True)
            return
        update_price("يوم", DEFAULT_PRICES["يوم"])
        update_price("اسبوع", DEFAULT_PRICES["اسبوع"])
        update_price("شهر", DEFAULT_PRICES["شهر"])
        bot.edit_message_text("✅ تم إعادة تعيين الأسعار إلى القيم الافتراضية!", call.message.chat.id, call.message.message_id, reply_markup=back_to_admin())
        bot.answer_callback_query(call.id)
        return
...
def process_update_price(message, plan_type):
    user_id = message.from_user.id
    if not is_admin(user_id):
        return
    ...
    try:
        ...
        if new_price < 0:
            bot.reply_to(message, "❌ السعر يجب أن يكون رقم موجب.", reply_markup=back_to_admin())
            return
        
        if update_price(plan_type, new_price):
            day_price = get_price("يوم")
            week_price = get_price("اسبوع")
            month_price = get_price("شهر")
            ...
            bot.reply_to(
                message,
                f"✅ تم تحديث سعر {plan_type} إلى {new_price} نجوم!\n\n"
                f"📅 الأسعار الحالية:\n"
                f"• يوم: {day_price} ⭐\n"
                f"• أسبوع: {week_price} ⭐\n"
                f"• شهر: {month_price} ⭐",
            ...
            bot.reply_to(message, "❌ فشل في تحديث السعر", reply_markup=back_to_admin())
    except ValueError:
        bot.reply_to(message, "❌ يرجى إرسال رقم صحيح.", reply_markup=back_to_admin())
...
def handle_duration(call):
    ...
    price_map = {"day": get_price("يوم"), "week": get_price("اسبوع"), "month": get_price("شهر"), "unlimited": 0}
    cost = price_map.get(duration_type, 0)
    ...
    if duration_type == "unlimited" or is_admin(user_id):
        ...
        # طلب الدفع بالنجوم باستخدام نظام Telegram Stars الرسمي
        add_pending_payment(user_id, bot_id, cost, duration_type)
        
        # إنشاء طلب دفع رسمي
        payment_text = f"""
⭐ <b>شحن نقاط - استضافة كايو</b>

━━━━━━━━━━━━━━━━━━
<b>مستخدماً شهرياً</b>
{stats_data['stats'].get('total_users', 0)}
...
━━━━━━━━━━━━━━━━━━
<b>طريقة الدفع:</b>
• اضغط على زر "دفع {cost} نجوم" أدناه
• سيتم خصم النجوم من رصيدك
• سيتم تفعيل بوتك فوراً

━━━━━━━━━━━━━━━━━━
⚠️ سيتم تحويل النجوم إلى المطور تلقائياً
"""
        
        keyboard = telebot.types.InlineKeyboardMarkup(row_width=1)
        keyboard.add(
            telebot.types.InlineKeyboardButton(
                text=f"💳 دفع {cost} نجوم",
                pay=True
            )
        )
        keyboard.add(
        ...
        try:
            bot.send_invoice(
                call.message.chat.id,
                title=f"شحن نقاط - استضافة كايو",
                description=f"شراء {duration_type} لتشغيل البوت {bot_name}",
                provider_token="",
                currency="XTR",
                prices=[telebot.types.LabeledPrice(label=f"{duration_type} - {bot_name}", amount=cost * 100)],
                start_parameter=f"bot_payment_{bot_id}",
                invoice_payload=f"bot_payment_{bot_id}_{user_id}",
                reply_markup=keyboard
            )
        except Exception as e:
            logger.error(f"خطأ في إنشاء طلب الدفع: {e}")
            bot.edit_message_text(
                f"❌ حدث خطأ في نظام الدفع. يرجى المحاولة مرة أخرى.",
                call.message.chat.id,
                call.message.message_id,
                reply_markup=back_button()
            )
...
def handle_publish_duration(call):
    ...
    # الحصول على الأسعار من قاعدة البيانات
    day_price = get_price("يوم")
    week_price = get_price("اسبوع")
    month_price = get_price("شهر")
    
    duration_map = {
        "publish_day": {"days": 1, "stars": day_price, "name": "يوم", "plan": "day"},
        "publish_week": {"days": 7, "stars": week_price, "name": "أسبوع", "plan": "week"},
        "publish_month": {"days": 30, "stars": month_price, "name": "شهر", "plan": "month"}
    }
...
"""
    ...
    # للمستخدمين العاديين - عرض طلب دفع رسمي
    add_pending_payment(user_id, "publish_subscription", duration['stars'], duration['plan'])
...
<b>📌 تفاصيل الشراء:</b>
• الباقة: {duration['name']}
• المدة: {duration['days']} يوم
• السعر: {duration['stars']} ⭐

━━━━━━━━━━━━━━━━━━
<b>طريقة الدفع:</b>
• اضغط على زر "دفع {duration['stars']} نجوم" أدناه
• سيتم خصم النجوم من رصيدك
• سيتم تفعيل اشتراكك فوراً
...
"""
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton(
            text=f"💳 دفع {duration['stars']} نجوم",
            pay=True
        )
    )
    keyboard.add(
    ...
    try:
        bot.send_invoice(
            call.message.chat.id,
            title=f"شحن نقاط - استضافة كايو",
            description=f"شراء باقة {duration['name']} للاشتراك في الاستضافة",
            provider_token="",
            currency="XTR",
            prices=[telebot.types.LabeledPrice(label=f"باقة {duration['name']}", amount=duration['stars'] * 100)],
            start_parameter=f"subscription_{duration['plan']}",
            invoice_payload=f"subscription_{duration['plan']}_{user_id}",
            reply_markup=keyboard
        )
    except Exception as e:
        logger.error(f"خطأ في إنشاء طلب الدفع: {e}")
...
def handle_pre_checkout(query):
    ...
    try:
        ...
        logger.error(f"خطأ في معالجة ما قبل الدفع: {e}")
        bot.answer_pre_checkout_query(query.id, ok=False, error_message="حدث خطأ، يرجى المحاولة مرة أخرى")
...
def handle_successful_payment(message):
    ...
    logger.info(f"✅ دفع ناجح من {user_id} - المبلغ: {amount} نجوم - البايلود: {invoice_payload}")
    ...
    try:
        bot.send_message(
            STAR_RECEIVER_ID,
            f"⭐ **تم استلام نجوم**\n\n"
            f"👤 من: {message.from_user.full_name} (@{message.from_user.username or 'لا يوجد'})\n"
            f"🆔 ايدي: `{user_id}`\n"
            f"💰 المبلغ: {amount} نجوم\n"
            f"📝 البايلود: `{invoice_payload}`\n"
            f"🕐 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            parse_mode="Markdown"
        )
        logger.info(f"✅ تم إشعار المطور باستلام {amount} نجوم من {user_id}")
    except Exception as e:
        logger.error(f"❌ خطأ في إشعار المطور: {e}")
    
    if invoice_payload.startswith("bot_payment_"):
        parts = invoice_payload.split("_")
        if len(parts) >= 3:
            bot_id = parts[2]
            activate_bot_payment(user_id, bot_id, amount)
    elif invoice_payload.startswith("subscription_"):
        parts = invoice_payload.split("_")
        if len(parts) >= 2:
            plan_type = parts[1]
            activate_subscription_payment(user_id, plan_type, amount)
    else:
        if amount >= 600:
            days = 30
            sub_type = "شهر"
...
def activate_bot_payment(user_id: int, bot_id: str, amount: int):
    """تفعيل البوت بعد الدفع"""
    pending = get_pending_payment(user_id)
    if not pending:
        bot.send_message(user_id, "❌ لا يوجد طلب دفع معلق")
        return
    
    if pending["bot_id"] != bot_id:
        bot.send_message(user_id, "❌ بيانات غير صحيحة")
        return
    ...
    if bot_data_db:
        ...
        duration_type = pending["plan_type"]
        ...
        if start_bot_process(bot_id, bot_file):
            update_bot_status(bot_id, "running")
            save_bot_to_db(bot_id, bot_name, user_id, bot_file, github_path, "running", duration_type, days, star_cost=pending["stars"])
            trigger_auto_deploy()
            ...
            bot.send_message(
                ...
                f"المستخدم: {user_id}\n"
                f"البوت: {bot_id} ({bot_name})\n"
                f"المبلغ: {pending['stars']} نجوم\n"
                f"المدة: {duration_type}"
            )
        else:
            bot.send_message(user_id, f"❌ فشل في تشغيل البوت {bot_id}")
    else:
        bot.send_message(user_id, "❌ البوت غير موجود")

def activate_subscription_payment(user_id: int, plan_type: str, amount: int):
    """تفعيل الاشتراك بعد الدفع"""
    days_map = {"day": 1, "week": 7, "month": 30}
    days = days_map.get(plan_type, 1)
    sub_type = {"day": "يوم", "week": "أسبوع", "month": "شهر"}.get(plan_type, "يوم")
    ...
    if add_user_subscription(user_id, days, sub_type):
        remove_pending_payment(user_id)
        ...
        bot.send_message(
            ...
            f"📅 المدة: {days} يوم\n"
            f"⭐ تم خصم {pending['stars']} نجوم\n\n"
            f"📌 يمكنك الآن رفع بوتك من خلال زر 'رفع بوت'",
            reply_markup=back_button()
        )
        ...
        bot.send_message(user_id, "❌ فشل في تفعيل الاشتراك")
...
if __name__ == "__main__":
    ...
    logger.info(f"⭐ حساب استلام النجوم: {STAR_RECEIVER_ID} (@ggzh9)")
    
    # عرض الأسعار الحالية
    day_price = get_price("يوم")
    week_price = get_price("اسبوع")
    month_price = get_price("شهر")
    logger.info(f"📅 الأسعار الحالية: يوم={day_price}, أسبوع={week_price}, شهر={month_price}")
    logger.info("=" * 60)
