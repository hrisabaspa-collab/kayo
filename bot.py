# bot.py - النسخة المحدثة مع نظام دفع Telegram Stars الرسمي
import telebot
import requests
import json
import os
...
def init_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS purchase_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, bot_id TEXT, plan_type TEXT, stars INTEGER, status TEXT, created_at TEXT)''')
...
"""
        ...
        # إنشاء زر الدفع باستخدام نظام Telegram Stars
        keyboard = telebot.types.InlineKeyboardMarkup(row_width=1)
        ...
        try:
            bot.send_invoice(
                ...
                provider_token="",  # فارغ لاستخدام النجوم
...
def handle_publish_duration(call):
    ...
    duration_map = {
        "publish_day": {"days": 1, "stars": 10, "name": "يوم", "plan": "day"},
        "publish_week": {"days": 7, "stars": 50, "name": "أسبوع", "plan": "week"},
        "publish_month": {"days": 30, "stars": 600, "name": "شهر", "plan": "month"}
    }
...
def handle_successful_payment(message):
    ...
    amount = payment.total_amount // 100  # تحويل إلى نجوم
    ...
    if invoice_payload.startswith("bot_payment_"):
        ...
        # دفع غير معروف - إضافة اشتراك عادي
        if amount >= 600:
            days = 30
            sub_type = "شهر"
        elif amount >= 50:
